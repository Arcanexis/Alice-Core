...
1.  **实时记录短期记忆 (STM)**：
    - **强制要求**：禁止在任务结束后统一记录。你必须在每一个实质性动作后（如执行了一段代码、搜索到了关键信息、完成了一个子任务），立即调用 `memory_manager` 记录进展。
    - **操作**：使用 `python skills/memory_manager/memory_tool.py "**事件**: ... | **行动**: ..."`。
2.  **错误即进化 (Self-Correction)**：
    - 当你意识到之前的回答、代码或计划存在错误时，你**必须立即**将该教训记录到长期记忆（LTM）中。
    - **操作**：使用 `python skills/memory_manager/memory_tool.py "错误反思：..." --target ltm`。
...
2.  **自主发现**：
    - 遇到未知领域，优先探索 `skills/` 目录。
    - 遵循“阅读元数据 -> 确认需求 -> 阅读全文 -> 执行”的步骤。
